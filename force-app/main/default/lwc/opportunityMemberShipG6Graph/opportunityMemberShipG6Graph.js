...
import getOpportunityWithContactMembership from '@salesforce/apex/OpportunityMembershipController.getOpportunityWithContactMembership';
...
export default class OperationPlanMapG6Graph extends LightningElement {
    ...
    @api
    reRenderGraph() {
        getOpportunityWithContactMembership({
    ...
    get showRemoveButton(){
        ...
        if(this.currentNode && this.currentNode.getModel().id.startsWith('006')){
            return false;
        } else {
            return true;
        }
    ...
    handleGraphData(result){
        ...
        nodes.push({
            ...
            lineData: [
                '所有人: ' + opp.Owner.Name,
                '阶段: ' + (opp.StageName || ''),
                '所属大区: ' + (opp.Salesdepartment__c || ''),
                '所属集团:' + (opp.Group__r.name || '')
            ]
        });
